...
export async function PUT(request: Request, { params }: { params: { id: string } }) {
  ...
  try {
    ...
    // Основное изображение вакансии
    const file = formData.get('image');
    let savedImage: string | null = null;

    if (file && file instanceof Blob) {
      try {
        const storageRef = ref(storage, `vacancies/${newPlace}-${newTitle}/main/${file.name}`);
        const uploadTask = uploadBytesResumable(storageRef, file);

        uploadTask.on(
          "state_changed",
          (snapshot) => {
            console.log(`Uploading main image: ${file.name} - Progress: ${snapshot.bytesTransferred} / ${snapshot.totalBytes}`);
          },
          (err) => {
            console.error('Error uploading main image:', err);
            return NextResponse.json({ message: 'Error uploading main image', error: err.message }, { status: 500 });
          },
          async () => {
            try {
              savedImage = await getDownloadURL(storageRef); // Получаем URL загруженного изображения
              console.log('Main image uploaded:', savedImage);
            } catch (err: any) {
              console.error('Error fetching download URL for main image:', err);
              return NextResponse.json({ message: 'Error fetching main image URL', error: err.message }, { status: 500 });
            }
          }
        );
      } catch (error: any) {
        console.error('Error saving main image:', error);
        return NextResponse.json({ message: 'Error saving main image', error: error.message }, { status: 500 });
      }
    }

    let savedHomeImages: string[] = []; // Массив для хранения URL изображений жилья
    ...
    if (homeImages.length > 0) {
      try {
        for (const file of homeImages) {
          if (file instanceof Blob) {
            const storageRef = ref(storage, `vacancies/${newPlace}-${newTitle}/home/${file.name}`);
            const uploadTask = uploadBytesResumable(storageRef, file);

            uploadTask.on(
              "state_changed",
              (snapshot) => {
                console.log(`Uploading home image: ${file.name} - Progress: ${snapshot.bytesTransferred} / ${snapshot.totalBytes}`);
              },
              (err) => {
              ...
              async () => {
                try {
                  const fileUrl = await getDownloadURL(storageRef); // Получаем URL для домашнего изображения
                  savedHomeImages.push(fileUrl); // Добавляем URL в массив
                  console.log('Home image uploaded:', fileUrl);
                } catch (err) {
                  console.error('Error fetching download URL for home image:', err);
                }
              }
            );
          }
    ...
    try {
      const updatedVacancy = await Vacancies.findByIdAndUpdate(id, {
        ...
        homeImages: savedHomeImages.length > 0 ? savedHomeImages : oldVacancy.homeImages,
        image: savedImage || oldVacancy.image, // Используем URL изображения из Firebase
      }, { new: true });

      console.log('Vacancy updated:', updatedVacancy);
      return NextResponse.json({ message: "Вакансия обновлена", success: true }, { status: 200 });
    } catch (error: any) {
      console.error('Ошибка при обновлении вакансии:', error);
      return NextResponse.json({ message: 'Ошибка при обновлении вакансии', error: error.message }, { status: 500 });
    }
