...
// const MyMultiSelect: React.FC<MultiSelectProps> = ({
//   label,
...
//   }, []);
//   useEffect(() => {
//     // Логируем для отслеживания состояния
//     console.log("Updated selectedOptions (useEffect):", selectedOptions);
//   }, [selectedOptions]);

//   useEffect(() => {
//     // Когда prop value изменяется, обновляем состояние selectedOptions
...
interface MultiSelectProps {
  ...
  options: Option[];
  [x: string]: any; // Для дополнительных свойств
  onChange: (selected: string[]) => void; // onChange для передачи выбранных значений
  value: string[]; // Значения, которые уже выбраны
}
...
const MyMultiSelect: React.FC<MultiSelectProps> = ({
  ...
  const handleOptionToggle = (value: string) => {
    setSelectedOptions((prev) =>
      prev.includes(value)
        ? prev.filter((val) => val !== value) // Убираем из выбранных
        : [...prev, value] // Добавляем в выбранные
    );
  };
