...
const DropdownNotification = () => {
  ...
  useEffect(() => {
    const fetchTasks = async () => {
      try {
        const response = await fetch(`/api/tasks/${managerId}/forManager`);
                ...
                {tasks.length > 0 ? (
                  tasks.map((task: any) => (
                    <li key={task._id}>
                      ...
                      >
                        ...
                        <Badge variant={'outline'} className="bg-green-800 text-white">Назначена {task && task.createdAt ? formatDistanceToNow(new Date(task.createdAt), { addSuffix: true, locale: ru }) : "Дата не указана"}
                        </Badge>
                        <Checkbox
                        ...
                        <div className="text-sm flex flex-col gap-1">
                          <span className="text-black dark:text-white">{task.taskName}</span> Назначена задача для кандидата
                          <div onClick={() => toggleSidebar("viewCandidate", task.candidate)} className="cursor-pointer">
                            <span className="font-semibold underline">{task?.candidate?.name}</span>
                          </div>
                        </div>
                      </div>
                    </li>
                  ...
                  <li>
                    <Link
                      ...
                      href="#"
                    >
                      <p className="text-sm text-center">Задачи не найдены.</p>
                    </Link>
                  </li>
                )}
              </ul>
